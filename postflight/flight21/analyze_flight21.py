#!/usr/bin/env python3
"""
Quick flight21 correlation helper.
- Decodes GP Control spans from the xiao log
- Correlates GP Eval setRcData commands to INAV rcData in the blackbox CSV
- Searches forward in time to see when the commanded RC values appear in INAV
- Generates per-span overlays for position/velocity/quaternion and RC
Outputs per-span timing and rcData deltas so we can spot latency/interpretation issues.
"""
...
BLACKBOX_CSV = ROOT / "flight21-blackbox.csv"
XIAO_LOG = ROOT / "flight21-hb1-flight_log_2025-12-17T01-27-45.txt"
...
def parse_xiao_log() -> List[Span]:
    ...
    eval_re = re.compile(
        r"^#[0-9]+\s+([0-9]+)\s+([0-9]+)\s+[iwe]\s+GP Eval:.*setRcData=\[(\d+),(\d+),(\d+)\]"
    )
    state_re = re.compile(
...
def generate_overview_plots(spans: List[Span], bb: pd.DataFrame):
    for idx, span in enumerate(spans, 1):
        ...
        # Positions logged as GP-relative already; keep raw if needed
        dbg_df["pos_n"] = df["pos_n"]
        dbg_df["pos_e"] = df["pos_e"]
        dbg_df["pos_d"] = df["pos_d"]
        ...
        if not dbg_df.empty:
            axes[0].scatter(dbg_df["t_rel"], dbg_df["pos_n"], s=12, color=colors[0], marker="o", label="xiao N (GP State)")
            axes[0].scatter(dbg_df["t_rel"], dbg_df["pos_e"], s=12, color=colors[1], marker="o", label="xiao E (GP State)")
            axes[0].scatter(dbg_df["t_rel"], dbg_df["pos_d"], s=12, color=colors[2], marker="o", label="xiao D (GP State)")
